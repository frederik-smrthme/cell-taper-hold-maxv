...
6. Bei Bedarf unter Einstellungen → Geräte & Dienste → Venus Cell Taper → **Konfigurieren** die Mindestleistung ändern (10 bis 50 W in 10-W-Schritten, Vorgabe 40 W).
...
Entpacke das ZIP und lade **den Inhalt** des Ordners `venus-cell-taper` in ein neues Repository namens `venus-cell-taper`. In `manifest.json` kannst du anschließend `OWNER` durch deinen GitHub-Namen ersetzen. Für HACS ist eine GitHub-Release nach dem ersten Commit sinnvoll; das Repository lässt sich auch als benutzerdefiniertes Repository hinzufügen.
